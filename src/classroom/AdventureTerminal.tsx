...
export default function AdventureTerminal({data,run,preview=false,onAppCommand,toolbar,messages}:{data:Snapshot;run:Run;preview?:boolean;onAppCommand?:(cmd:string)=>boolean;toolbar?:ReactNode;messages?:ReactNode}) {
 ...
 const act=(raw:string)=>{
   ...
   execute(async()=>{
     ...
     else if(['동','서','남','북','봐라','회복','대화','읽기','열기','입장','퇴장'].includes(cmd)) {const r=await rpc('explore',{p_command:cmd==='퇴장'?'나가기':cmd,p_request:crypto.randomUUID()});write(r.text.replaceAll('「나가기」','「퇴장」'));}
 ...
 {direction('북')}<span className="compass-up">↑</span><div className="compass-middle">{direction('서')}<span>←</span><strong title={loc?.name}>현 위치</strong><span>→</span>{direction('동')}</div><span className="compass-down">↓</span>{direction('남')}
 ...
 <div className="ps-output">{messages}<p className="ps-location-description">{loc?.description?.replaceAll('나가기로','퇴장으로')}</p>{lines.map(l=><div key={l.id}><Stream text={l.text}/></div>)}
