...
export type Battle = { id: string; monster: string; level?: number; distance?: number; hp: number; correct_count: number; token: string; repeated: boolean; question: Pick<Question,'id'|'kind'|'body'|'options'|'subject'> };
export type InventoryItem = {id:string;student:string;entity:string;name:string;data:Record<string,any>;equipped:boolean;slot:string;acquired_at:string};
export type Snapshot = { profile: Profile; students: Profile[]; settings: Settings; questions: Question[]; proposals: Proposal[]; places: Place[]; links: Link[]; entities: Entity[]; inventory:InventoryItem[]; ledger: Ledger[]; battle: Battle|null };
export const START = '00000000-0000-4000-8000-000000000001';
export const categories: Record<string,string> = {square:'광장',village:'마을',house:'주택',road:'길·다리',forest:'숲·황야',dungeon:'던전',healing:'치유소',library:'도서관',castle:'성'};
export const labels: Record<string,string> = {ox:'OX',short:'단답형',choice:'객관식',pending:'승인 대기',submitted:'검토 대기',approved:'승인',revision:'수정 요청',rejected:'반려',draft:'작성 중',suspended:'이용 중지',cancelled:'취소',map:'맵',monster:'몬스터',item:'아이템',skill:'기술',npc:'NPC',book:'책',heal:'회복',guard:'방어',strike:'데미지',none:'없음',weapon:'무기',armor:'방어구',accessory:'장신구',consumable:'소모품'};
export const defaults: Settings = {id:true,subjects:['국어','수학','사회','과학','실과'],question_reward:10,daily_reward_cap:100,room_cost:10,stat_cost:10,stat_cap:100,monster_base:10,effect_base:10,npc_cost:10,book_cost:5,adventure_daily_cap:10,loot_chance:40,encounter_chance:40,book_reward_chance:30,registration_open:true,creation_open:true,paused:false};
...
export function estimate(kind: string, d: Record<string,any>, s: Settings) {
 ...
 if(kind==='npc'||kind==='book') return (kind==='npc'?s.npc_cost:s.book_cost)+Math.max(0,Math.ceil(Array.from(d.description||'').length/500)-1);
 if(kind==='monster') return s.monster_base+Math.ceil(Number(d.hp)/10)+Number(d.attack)+Number(d.defense)+Number(d.level)*2+Number(d.power||0)*2;
 if(kind==='item') return s.effect_base+Math.ceil(Number(d.hp||0)/5)+Number(d.attack||0)*2+Number(d.defense||0)*2+Number(d.power||0)*2;
...
export function previewSnapshot(role:'teacher'|'student'): Snapshot {
 ...
 links:[{source:START,direction:'동',target:'forest'},{source:'forest',direction:'서',target:START}],entities:[{id:'preview-m',creator:student.id,kind:'monster',name:'물음표 슬라임',data:{hp:70,attack:10,defense:2,level:1,min_correct:3},active:true,place:'forest'}],inventory:[],ledger:[],battle:null};
}
